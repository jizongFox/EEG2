...
if __name__ == '__main__':
    ...
    for i in np.unique(subj_nums):
        ...
        fold_pairs.append((tr, ts)) # outer patients, one certain patient.
    ...
    av_feats = av_feats / int(feats.shape[1] / 192)
    images = gen_images(np.array(locs_2d),
                                  av_feats,
                                  32, normalize=False)
    print('\n')
    # from sklearn.preprocessing import minmax_scale
    # mms = minmax_scale(images[0][0],feature_range=(0,1))
    # images_=mms.fit_transform(images)
    ...
    images_timewin = np.array([gen_images(np.array(locs_2d),feats[:, i * 192:(i + 1) * 192], 32, normalize=False) for i in
                                         range(int(feats.shape[1] / 192))
                                         ])
    print('\n')
    print('Training the LSTM-CONV Model...')
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[2], 'maxpool')
